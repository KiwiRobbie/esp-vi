        ...
        struct timeval *tp)
{
        ...
        {
                nr = uart_read_bytes(UART_NUM_0, bp, blen, 20 / portTICK_PERIOD_MS);
                ESP_LOGI("cl_read", "bytes read: %d", nr);
                switch (nr)
                {
...
#define ONE_FOR_EOF 1
        ...
        while (nr == 0)
        {
                nr = uart_read_bytes(UART_NUM_0, bp, blen - ONE_FOR_EOF, 10 / portTICK_PERIOD_MS);
                // ESP_LOGI("cl_read", "buffer size: %d number read: %d", blen - ONE_FOR_EOF, nr);
        }
        ...
        case 0: /* EOF. */
                ...
                if (++clp->eof_count < 50)
                {
                        ESP_LOGI("cl_read", "EOF count: %d", clp->eof_count);
