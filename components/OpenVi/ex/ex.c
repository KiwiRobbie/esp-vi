...
skip_srch:      if (ecp->cmd == &cmds[C_VISUAL_EX] && F_ISSET(sp, SC_VI))
        ...
        //         }
                ...
                if (isalnum((int)ecp->cp[0]) || ecp->cp[0] == '|') {
                        ecp->rcmd = cmds[C_SUBSTITUTE];
                        ecp->rcmd.fn = ex_subagain;
                        ecp->cmd = &ecp->rcmd;
                } else if (ecp->clen > 0) {
...
search:         mp->lno = sp->lno;
        ...
        case '.':                               /* Current position. */
                *isaddrp = 1;
                 ...
                 * as well.)
                 */
                if (ecp->clen > 1 && isdigit((int)ecp->cp[1]))
                        *ecp->cp = '+';
                else {
                        ++ecp->cp;
                        --ecp->clen;
         ...
         */
        total = 0;
        if (ecp->clen != 0 && (isdigit((int)ecp->cp[0]) ||
            ecp->cp[0] == '+' || ecp->cp[0] == '-' ||
            ecp->cp[0] == '^')) {
                if (!*isaddrp) {
                        *isaddrp = 1;
