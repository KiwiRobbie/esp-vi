...
static const char *LOGGING_TAG = "exf";
...
file_add(SCR *sp, CHAR_T *name)
{
        ...
        if (name != NULL)
                TAILQ_FOREACH_SAFE(frp, &gp->frefq, q, tfrp)
                {
                        ...
                        if (!strcmp(frp->name, (const char *)name))
                                return (frp);
                }
         ...
         */
        if (name != NULL && strcmp((const char *)name, TEMPORARY_FILE_STRING) &&
            (frp->name = strdup((const char *)name)) == NULL)
        {
                free(frp);
...
int file_init(SCR *sp, FREF *frp, char *rcv_name, int flags)
{
        ...
        ESP_LOGI(LOGGING_TAG, "Starting file_init");
        ...
        ESP_LOGI(LOGGING_TAG, "Prepare file scan");
        ...
        {
                free(ep);
                ESP_LOGI(LOGGING_TAG, "Fail file scan");
                return (1);
        }
        ESP_LOGI(LOGGING_TAG, "Pass file scan");
        /*
        ...
        {
                free(ep);
                ESP_LOGI(LOGGING_TAG, "Fail exists");
                return (1);
        }

        ESP_LOGI(LOGGING_TAG, "Block start");
        if (LF_ISSET(FS_OPENERR) || oname == NULL || !exists)
        {
                ...
                if (frp->tname != NULL)
                {
                        ESP_LOGI(LOGGING_TAG, "Don't try to create a temporary support file twice.");
                        goto err;
                }
                ESP_LOGI(LOGGING_TAG, "Trying to create tmp file: %s", tname);
                // fd = mkstemp(tname);
                FILE *file = fopen(tname, "w");
                fd = fileno(file);

                // const char *buffer = malloc(1024);
                // memset(&sb, 0, sizeof(sb));
                // sb.st_mode = S_IRWXU | S_IRWXG | S_IRWXO | S_IFREG;
                // sb.st_size = 1024;
                // sb.st_blksize = 1024;

                ESP_LOGI(LOGGING_TAG, "Opened file fd=%d", fd);

                int fstat_ret = fstat(fd, &sb);
                if (fd == -1 || fstat_ret == -1)
                {
                        ESP_LOGI(LOGGING_TAG, "Unable to create temporary file fd=%d, fstat=%d", fd, fstat_ret);
                        msgq(sp, M_SYSERR,
        ...
        ESP_LOGI(LOGGING_TAG, "Save device, inode and modification time.");
        ...
        ESP_LOGI(LOGGING_TAG, "Set up recovery.");
        ...
        ESP_LOGI(LOGGING_TAG, "Open a db structure.");
        /*
...
int file_write(SCR *sp, MARK *fm, MARK *tm, char *name, int flags)
{
        ...
        {
                msgq(sp, M_ERR, LF_ISSET(FS_POSSIBLE) ? "Read-only file, not written; use ! to override" : "Read-only file, not written");
                return (1);
        }
        ...
        if (!LF_ISSET(FS_FORCE | FS_APPEND) && !O_ISSET(sp, O_WRITEANY))
        {
                    ...
                    !stat(name, &sb))
                {
                        msgq_str(sp, M_ERR, name,
                                 LF_ISSET(FS_POSSIBLE) ? "%s exists, not written; use ! to override" : "%s exists, not written");
                        return (1);
                }
                ...
                {
                        msgq(sp, M_ERR, LF_ISSET(FS_POSSIBLE) ? "Partial file, not written; use ! to override" : "Partial file, not written");
                        return (1);
                }
        }
        ...
        else
        {
                ...
                {
                        msgq_str(sp, M_ERR, name, LF_ISSET(FS_POSSIBLE) ? "%s: file modified more recently than this copy; use ! to override" : "%s: file modified more recently than this copy");
                        return (1);
                }
        ...
        case NEWFILE:
                ...
                break;
        case OLDFILE:
                msgstr = LF_ISSET(FS_APPEND) ? "%s: appended: %'lu lines, %'lu characters" : "%s: %'lu lines, %'lu characters";
...
file_backup(SCR *sp, char *name, char *bname)
{
        ...
        //                         goto err;
        //                 for (t = bp, slash = NULL,
        //                     p = (char*)cmd.argv[0]->bp; p[0] != '\0'; *t++ = *p++)
        //                         if (p[0] == '%') {
        //                                 if (p[1] != '%')
        //                                         *t++ = '%';
        //                         } else if (p[0] == '/')
...
int file_m1(SCR *sp, int force, int flags)
{
        ...
        if (F_ISSET(ep, F_MODIFIED))
        {
                ...
                {
                        msgq(sp, M_ERR, LF_ISSET(FS_POSSIBLE) ? "File may be modified since last complete write; write or use ! to override" : "File may be modified since last complete write; write or use :edit! to override");
                        return (1);
                }
        }
