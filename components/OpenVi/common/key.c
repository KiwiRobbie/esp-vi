 ...
 */
int v_key_val(SCR *sp, CHAR_T ch)
{
        KEYLIST k, *kp;
 ...
 */
int v_event_push(SCR *sp, EVENT *p_evp, CHAR_T *p_s, size_t nitems, unsigned int flags)
{
        EVENT *evp;
        GS *gp;
        size_t total;
 ...
 */
int v_event_get(SCR *sp, EVENT *argp, int timeout, u_int32_t flags)
{
        EVENT *evp, ev;
        GS *gp;
        SEQ *qp;
...
newmap:
        ...
        if (ispartial)
        {
                if (O_ISSET(sp, O_TIMEOUT))
                        timeout = (evp->e_value == K_ESCAPE ? O_VAL(sp, O_ESCAPETIME) : O_VAL(sp, O_KEYTIME)) * 100;
                else
                        timeout = 0;
                goto loop;
        }
        ...
        {
        nomap:
                if (!isdigit(evp->e_c) && LF_ISSET(EC_MAPNODIGIT))
                        goto not_digit;
                *argp = *evp;
                QREM(1);
                return (0);
 ...
 */
void v_event_err(SCR *sp, EVENT *evp)
{
        switch (evp->e_event)
        {
        case E_CHARACTER:
 ...
 */
int v_event_flush(SCR *sp, unsigned int flags)
{
        GS *gp;
        int rval;

        for (rval = 0, gp = sp->gp; gp->i_cnt != 0 &&
                                    F_ISSET(&gp->i_event[gp->i_next].e_ch, flags);
             rval = 1)
                QREM(1);
        return (rval);
}
