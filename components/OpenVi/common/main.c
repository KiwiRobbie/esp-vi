...
int editor(GS *gp, int argc, char *argv[])
{
        ...
        ESP_LOGI("editor", "Started Editor: %d", argc);
        ...
        {
                perror("pledge");
                ESP_LOGI("editor", "bsd pledge error");
                goto err;
        }
        ...
        ESP_LOGI("editor", "Common global structure initialization.");
        ...
        ESP_LOGI("editor", "CStructures shared by screens so stored in the GS structure.");
        ...
        ESP_LOGI("editor", "Set initial screen type and mode based on the program name.");
        ESP_LOGI("editor", "name: %s", bsd_getprogname());
        ESP_LOGI("editor", "ro: %d", readonly);

        /* Convert old-style arguments into new-style ones. */
        if (v_obsolete(argv))
        {
                ESP_LOGI("editor", "Error raised by v_obsolete");
                return (1);
        }
        ...
        if (screen_init(gp, NULL, &sp))
        {
                ...
                ESP_LOGI("editor", "Screen init failed");
                goto err;
        }
        F_SET(sp, SC_EX);
        TAILQ_INSERT_HEAD(&gp->dq, sp, q);
        ...
        ESP_LOGI("editor", "Prepare to read EXINIT, exrc files.");
        ...
        ESP_LOGI("editor", "Read EXINIT, exrc files.");
        ...
        ESP_LOGI("editor", "Used remaining args as file names");
        ...
        if (sp->frp == NULL)
        {
                if (sp->argv == NULL)
                {
                        ESP_LOGI("editor", "Creating tmp file");

                        if ((frp = file_add(sp, NULL)) == NULL)
                        {
                                ESP_LOGI("editor", "Failed to create tmp file");
                                goto err;
                        }
                }
                else
                {
                        ...
                        {
                                ESP_LOGI("editor", "Failed to create file: %s", sp->argv[0]);
                                goto err;
                        }
                ...
                {
                        ESP_LOGI("editor", "Failed to init file");
                        goto err;
                }
                if (EXCMD_RUNNING(gp))
                {
                        ESP_LOGI("editor", "EXCMD_RUNNING failed check");
                        ...
                        if (F_ISSET(sp, SC_EXIT | SC_EXIT_FORCE))
                        {
                                if (screen_end(sp))
                                {
                                        ESP_LOGI("editor", "F_ISSET SC_EXIT |  SC_EXIT_FORCE");
                                        goto err;
                                }
                                goto done;
                        }
        ...
        ESP_LOGI("editor", "Reached main loop");

        while (sp != NULL)
        {
                ESP_LOGI("main_loop", "Loop");
                if (F_ISSET(sp, SC_EX) ? ex(&sp) : vi(&sp))
                {
                        ESP_LOGI("main_loop", "Err");
                        goto err;
                }
        }
        ...
        rval = 0;
        ESP_LOGI("editor", "Finished normally");
        if (0)
        {
        err:
                rval = 1;
                ESP_LOGE("editor", "Finished with Error");
        }
