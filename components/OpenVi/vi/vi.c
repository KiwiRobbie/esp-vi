...
static VIKEYS const *v_alias(SCR *, VICMD *, VIKEYS const *);
...
int vi(SCR **spp)
{
        ...
        for (vip = VIP(sp), rval = 0;;)
        {
                 ...
                 */
                int cmd_result = v_cmd(sp, DOT, vp, NULL, &comcount, &mapped);
                ESP_LOGI("vi", "v_cmd(...) = %d", cmd_result);
...
v_keyword(SCR *sp)
{
        ...
        for (moved = 0,
            beg = sp->cno;
             beg < len && isspace((int)p[beg]); moved = 1, ++beg)
                ;
        if (beg >= len)
        {
        ...
        for (state = inword((int)p[beg]),
            end = beg;
             ++end < len && state == inword((int)p[end]);)
                ;
...
v_alias(SCR *sp, VICMD *vp, VIKEYS const *kp)
{
        ...
        return (v_event_push(sp,
                             NULL, &push, 1, CH_NOMAP | CH_QUOTED)
                    ? NULL
                    : &vikeys[vp->key]);
}
