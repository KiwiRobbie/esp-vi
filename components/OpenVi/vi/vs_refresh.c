 ...
 */
int vs_repaint(SCR *sp, EVENT *evp)
{
        SMAP *smp;
 ...
 */
int vs_refresh(SCR *sp, int forcepaint)
{
        GS *gp;
        SCR *tsp;
        int need_refresh;
        ...
        {
                (void)vs_paint(tsp,
                               (F_ISSET(VIP(tsp), VIP_CUR_INVALID) ? UPDATE_CURSOR : 0) | UPDATE_SCREEN);
        ...
        if (vs_paint(sp, UPDATE_CURSOR | (!forcepaint &&
                                                  F_ISSET(sp, SC_SCR_VI) && KEYS_WAITING(sp)
                                              ? 0
                                              : UPDATE_SCREEN)))
                return (1);
...
adjust:
         ...
         */
fast:
        (void)gp->scr_cursor(sp, &y, &notused);
        goto done_cursor;
...
number:
        ...
        if (LF_ISSET(UPDATE_SCREEN))
                (void)gp->scr_refresh(sp, 0); // F_ISSET(vip, VIP_N_EX_PAINT));
