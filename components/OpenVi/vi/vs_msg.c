 ...
 */
void vs_busy(SCR *sp, const char *msg, busy_t btype)
{
        GS *gp;
        VI_PRIVATE *vip;
 ...
 */
void vs_update(SCR *sp, const char *m1, const char *m2)
{
        GS *gp;
 ...
 */
void vs_msg(SCR *sp, mtype_t mtype, char *line, size_t len)
{
        GS *gp;
        VI_PRIVATE *vip;
...
{
        ESP_LOGI("vs_output", "called vs_output() %s", line);
 ...
 */
int vs_ex_resolve(SCR *sp, int *continuep)
{
        EVENT ev;
        GS *gp;
        VI_PRIVATE *vip;
        ...
        *continuep = 0;
        ...
        else if (!F_ISSET(sp, SC_EX_WAIT_YES) && vip->totalcount < 2)
        {
                F_CLR(sp, SC_EX_WAIT_NO);
                return (0);
        }
 ...
 */
int vs_resolve(SCR *sp, SCR *csp, int forcewait)
{
        EVENT ev;
        GS *gp;
        MSGS *mp;
...
vs_scroll(SCR *sp, int *continuep, sw_t wtype)
{
        ...
        if (!IS_ONELINE(sp))
        {
                 ...
                 */
                (void)gp->scr_move(sp, vip->totalcount < sp->rows ? LASTLINE(sp) - vip->totalcount : 0, 0);
